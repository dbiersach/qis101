...
def train(init_strategy, learning_rate=1.0):
    ...
    final_hidden_output = sigmoid(np.dot(x, W_ih))
    final_output = sigmoid(np.dot(final_hidden_output, W_ho))
...
def plot_models():
    ...
    plt.title(f"XOR NN (2-4-1) w/o Bias ({EPOCHS:,} Epochs)\nLearning Rate vs. Loss")
